...
# Seed data from frontend - must stay in sync with src/data/courses.ts
SEED_COURSES = [
    {
        "id": "general",
        "code": "CV-GEN",
        ...
        "key_skills": ["Admission Guidance", "Fee Breakdown", "Placement Cell Support", "Batch Schedules", "Eligibility Criteria"],
        "sample_questions": [
    ...
    },
    {
        ...
        "key_skills": ["Embedded C", "ARM Cortex Architecture", "FreeRTOS", "Linux System Calls", "I2C / SPI / CAN / UART", "GDB & JTAG Debugging"],
        "sample_questions": [
    ...
    },
    {
        ...
        "key_skills": ["Verilog HDL", "SystemVerilog (OOP & Assertions)", "UVM Methodology", "FPGA Prototyping", "Static Timing Analysis (STA)", "ModelSim / QuestaSim"],
        "sample_questions": [
    ...
    },
    {
        ...
        "key_skills": ["Classic AUTOSAR", "CAN / CAN-FD Protocol", "LIN & FlexRay", "UDS (ISO 14229)", "ISO 26262 ASIL", "Vector CANoe & CAPL"],
        "sample_questions": [
    ...
    },
    {
        ...
        "key_skills": ["ESP32 (ESP-IDF)", "MQTT / CoAP / HTTP REST", "BLE 5.0 & Mesh", "AWS IoT Core / Azure IoT", "TinyML (TensorFlow Lite for Microcontrollers)", "Edge AI Quantization"],
        "sample_questions": [
    ...
    },
    {
        ...
        "key_skills": ["Linux Kernel Modules", "Char & Platform Drivers", "Device Tree (DTS)", "Interrupt Handlers & Workqueues", "Kernel Synchronization (Spinlocks)", "GDB Kernel Debugging (KGDB)"],
        "sample_questions": [
...
async def ensure_seed_courses(db: AsyncSession) -> None:
...
async def get_course(db: AsyncSession, course_id: str) -> Course | None:
    ...
    if course is None:
        # Ensure courses are seeded if table is empty
        await ensure_seed_courses(db)
        result = await db.execute(select(Course).where(Course.id == course_id))
        course = result.scalar_one_or_none()
    return course
